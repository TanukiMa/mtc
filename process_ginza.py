...
def main():
    ...
    # ▼▼▼▼▼ 環境変数からモデルパスを取得 ▼▼▼▼▼
    ginza_model_path = os.environ.get("GINZA_MODEL_PATH")
    if not ginza_model_path:
        raise ValueError("GINZA_MODEL_PATH environment variable is not set.")
    # ▲▲▲▲▲ ここまで追加 ▲▲▲▲▲
    ...
    while True:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            # ▼▼▼▼▼ ワーカーにモデルパスを渡す ▼▼▼▼▼
            futures = [executor.submit(worker_process_url, item, supabase_url, supabase_key, req_timeout, req_delay, debug_mode, ginza_model_path) for item in res.data]
