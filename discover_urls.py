...
def main():
    ...
    start_url_from_config = url_normalize(config.get('General', 'START_URL'))
    ...
    urls_to_visit.add(start_url_from_config)
    urls_to_visit.update(index_pages)
    
    if sitemap_url:
        sitemap_links = parse_sitemap(sitemap_url, session)
        normalized_sitemap_links = {url_normalize(link) for link in sitemap_links}
        urls_to_visit.update(normalized_sitemap_links)
