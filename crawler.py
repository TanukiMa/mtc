...
def main():
    ...
    try:
        supabase.table("crawled_urls").delete().lt("crawled_at", threshold_date.isoformat()).execute()
    except Exception as e:
        print(f"  [!] 古いURLの削除中にエラーが発生しました: {e}")
    
    try:
        response = supabase.table("crawled_urls").select("url", count='exact').execute()
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        while urls_to_crawl and processed_count < MAX_URLS_TO_CRAWL:
            ...
            future_to_url = {executor.submit(process_url, url): url for url in urls_now}
            for future in future_to_url:
                ...
                try:
                    ...
                    supabase.table("crawled_urls").insert({"url": url}).execute()
                    processed_count += 1
                    if processed_count >= MAX_URLS_TO_CRAWL:
                        break
                except Exception as e:
                    print(f"[!] future.result()でエラー: {e}")
